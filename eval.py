...
word_map_file = (
    data_folder + "/WORDMAP_flickr8k_5_cap_per_img_5_min_word_freq.json"
...
def evaluate(beam_size):
    ...
    loader = torch.utils.data.DataLoader(
        CaptionDataset(
            data_folder, data_name, "TEST", transform=transforms.Compose([normalize])
        ),
        batch_size=1,
        shuffle=True,
        num_workers=1,
        ...
        img_captions = list(
            map(
                lambda c: [
                    ...
                    for w in c
                    if w
                    not in {word_map["<start>"], word_map["<end>"], word_map["<pad>"]}
                ],
                img_caps,
            )
